...
<head>
    ...
    <link rel="stylesheet"
        href="{{ asset('assets/theme2/css/' . (!empty($store->store_theme) ? $store->store_theme : 'green-color.css')) }}">
...
<body>
    ...
    <header
        class="header {{ Request::segment(3) != '' ? 'inner-page' : '' }}{{ Request::segment(1) === 'page' ? 'inner-page' : '' }}{{ Request::segment(2) === 'customer-login' ? 'inner-page' : '' }}"
        id="header-main">
        <!-- Main navbar -->
            ...
            <div class="container px-lg-0">
                ...
                <a class="navbar-brand mr-lg-5" href="{{ route('store.slug', $store->slug) }}">
                    ...
                    @if (route('store.slug'))
                        ...
                        @if (!empty($store->logo))
                            <img alt="Image placeholdergr"
                                src="{{ asset(Storage::url('uploads/store_logo/' . $store->logo)) }}" id="navbar-logo"
                                class="navbar-logo" style="height: 40px;">
                        @else
                            <img alt="Image placeholdergr"
                                src="{{ asset(Storage::url('uploads/store_logo/logo-2.png')) }}" id="navbar-logo"
                                class="navbar-logo" style="height: 40px;">
                        @endif
                    ...
                    @else
                        @if (!empty($store->logo))
                            <img alt="Image placeholderrgrg"
                                src="{{ asset(Storage::url('uploads/store_logo/' . route('store.slug') ? 'logo-2.png' : $store->logo)) }}"
                                id="navbar-logo" class="navbar-logo" style="height: 40px;">
                            <img alt="Image placeholder" class="mobile-view"
                                src="{{ asset(Storage::url('uploads/store_logo/' . $store->logo)) }}" id="navbar-logo">
                        @else
                            <img alt="Image placeholdergrg"
                                src="{{ asset(Storage::url('uploads/store_logo/logo.png')) }}" id="navbar-logo"
                                class="navbar-logo" style="height: 40px;">
                            <img alt="Image placeholdergrg"
                                src="{{ asset(Storage::url('uploads/store_logo/logo.png')) }}" class="mobile-view"
                                id="navbar-logo">
                        @endif
                    @endif
                </a>
                ...
                <div class="collapse navbar-collapse" id="navbar-main-collapse">
                    <ul class="navigation-menu align-items-lg-center">
                        <!-- Home - Overview  -->
                        <li class="nav-item ">
                            <a class="nav-link {{ route('store.slug') ?: 'text-dark' }} {{ Request::segment(1) == 'store-blog' ? 'text-dark' : '' }}"
                    ...
                    <ul class="navbar-nav align-items-lg-center ml-lg-auto nav-my-store">
                        ...
                        @if (Utility::CustomerAuthCheck($store->slug) == true)
                            <div class="drop-down">
                                <div id="dropDown" class="drop-down__button ">
                                    <a
                                        class="nav-link nav-link btn ml-2 bg--gray hover-translate-y-n3 icon-font login_btn">{{ ucFirst(Auth::guard('customers')->user()->name) }}
                        ...
                        @else
                            <li class="nav-item">
                                <a href="{{ route('customer.login', $store->slug) }}"
                                    class="nav-link btn ml-2 bg--gray hover-translate-y-n3 icon-font login_btn">{{ __('Log in') }}</a>
                            </li>
                        @endif
            ...
            @if($getStoreThemeSetting[7]['section_enable'] == 'on')
                <div class="row pt-md top-footer delimiter-top">
                        ...
                        @if ($getStoreThemeSetting[7]['section_enable'] == 'on')
                            <div class="col-lg-6 mb-5 mb-lg-0">
                                <a href="{{ route('store.slug', $store->slug) }}">
                                    <img class="mb-3"
                                        src="{{ asset(Storage::url('uploads/' . $getStoreThemeSetting[7]['inner-list'][0]['field_default_text'])) }}"
                                        alt="Footer logo" style="height: 40px;">
                                </a>
                                <p class="t-l-gray w-75">
                                    {{ __('Nowadays, it isnt great uncommon to see lenders rapidly adopting a new digital') }}
                                </p>
                                <div class="get-qus">
                    ...
                    @if (!empty($getStoreThemeSetting[8]))
                        @if ((isset($getStoreThemeSetting[8]['section_enable']) && $getStoreThemeSetting[8]['section_enable'] == 'on') ||
                            ...
                            <div class="col-lg-2 col-sm-4 col-4 mb-5 mb-lg-0">
                                <h6 class="heading text-primary mb-3">
                                    {{ __($getStoreThemeSetting[8]['inner-list'][1]['field_default_text']) }}</h6>
...
@if($getStoreThemeSetting[10]['inner-list'][0]['field_default_text'] == 'on')
                    @if (!empty($getStoreThemeSetting[10]))
                        @if ((isset($getStoreThemeSetting[10]['section_enable']) &&
                            ...
                            <div class="col-lg-2 col-sm-4 col-4 mb-5 mb-lg-0">
                                <h6 class="heading text-primary mb-3">
                                    {{ __($getStoreThemeSetting[10]['inner-list'][1]['field_default_text']) }}</h6>
                                <ul class="list-unstyled f-list">
                                    @if (isset(
                    ...
                    @if (!empty($getStoreThemeSetting[12]))
                        @if ((isset($getStoreThemeSetting[12]['section_enable']) &&
                            ...
                            <div class="col-lg-2 col-sm-4 col-4 mb-5 mb-lg-0">
                                <h6 class="heading text-primary mb-3">
                                    {{ __($storethemesetting['quick_link_header_name3']) }}</h6>
                                <ul class="list-unstyled f-list">
                                    @if (isset(
