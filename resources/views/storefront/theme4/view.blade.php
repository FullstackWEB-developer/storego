                        ...
                        <div class="left-side">
                            ...
                            <div class="pd-rate">
                                <div class="p-rateing  d-flex">
                                    ...
                                    <p class="mb-0 ml-3"><span class="t-gray">{{ $avg_rating }}/5 ({{ $user_count }}
                                            reviews)</span></p>
                                </div>
                            </div>
                            <div class="product-price">
                                ...
                                <sup
                                    class="h3 mb-0 sub-price">{{ \App\Models\Utility::priceFormat($products->last_price) }}</sup>
                            </div>
                        </div>
                        <div class="center-side">
                            <div class="container">
                                ...
                                <div class="cart-buttons">
                                    <a href="#"
                                        class="btn btn-primary rounded-pill btn-icon shadow hover-translate-y-n3 add_to_cart"
                                        data-id="{{ $products->id }}">
                                    ...
                                    <div class="cat-id-block">
                                        <p class="mb-0 t-black14"><span class="t-gray">{{ __('Category') }}:</span>
                                            {{ $products->product_category() }}</p>
                                        <p class="mb-0 t-black14"><span class="t-gray">{{ __('ID') }}:</span>
                                            {{ $products->SKU }}</p>
                                    </div>
                                </div>
                                @if (!empty($products->custom_field_1) && !empty($products->custom_value_1))
                                    <div class="cart-buttons">
                                        <div class="mb-0 t-black14"><span class="t-gray">{{ $products->custom_field_1 }} :
                                            </span> {{ $products->custom_value_1 }}</div>
                                    </div>
                                @endif
                                @if (!empty($products->custom_field_2) && !empty($products->custom_value_2))
                                    <div class="cart-buttons">
                                        <div class="mb-0 t-black14"><span class="t-gray">{{ $products->custom_field_2 }} :
                                            </span> {{ $products->custom_value_2 }}</div>
                                    </div>
                                @endif
                                @if (!empty($products->custom_field_3) && !empty($products->custom_value_3))
                                    <div class="cart-buttons">
                                        <div class="mb-0 t-black14"><span class="t-gray">{{ $products->custom_field_3 }} :
                                            </span> {{ $products->custom_value_3 }}</div>
                                    </div>
                                @endif
                                @if (!empty($products->custom_field_4) && !empty($products->custom_value_4))
                                    <div class="cart-buttons">
                                        <div class="mb-0 t-black14"><span class="t-gray">{{ $products->custom_field_4 }} :
                                            </span> {{ $products->custom_value_4 }}</div>
                                    </div>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-lg-6">
                                    ...
                                    @if (!empty($products->attachment))
                                        <div class="button">
                                            <a href="{{ asset(Storage::url('uploads/is_cover_image/' . $products->attachment)) }}"
                                                class="text-primary btn-instruction"
                                                download="{{ $products->attachment }}">
                                                <span class="btn-inner--icon">
                                                    <i class="fas fa-shopping-basket"></i>
                                                </span>
                                ...
                                <div class="col-lg-6">
                                    <div class="product-right-img">
                                        @if (!empty($products->is_cover) && \Storage::exists('uploads/is_cover_image/' . $products->is_cover))
                                            <img src="{{ asset(Storage::url('uploads/is_cover_image/' . $products->is_cover)) }}"
                            ...
                            <div class="row align-items-center">
                                <div class="col-lg-6">
                                    <div class="bg-blue">
                                        @if (!empty($products->is_cover) && \Storage::exists('uploads/is_cover_image/' . $products->is_cover))
                                            <img src="{{ asset(Storage::url('uploads/is_cover_image/' . $products->is_cover)) }}"
                                                class="product-img" alt="#" style="width:525px; height: 343px ">
                                        @else
                                            <img src="{{ asset(Storage::url('uploads/is_cover_image/default.jpg')) }}"
                                                class="d-block w-100">
                                        @endif
                                ...
                                <div class="col-lg-6">
                                    <div class="customer-product-review">
                                        <div class="review-title">
                                            <h5><span class="r-title">{{ __('Reviews') }}:</span> <span
                                                    class="r-rate">{{ $avg_rating }}/5</span> <span class="t-gray">
                                                    ({{ $user_count }} {{ __('reviews') }})</span></h5>
                                            <div class="p-rateing  d-flex">
                                            ...
                                            @if ($product_rating->rating_view == 'on')
                                                ...
                                                <div class="pd-rate">
                                                    <div class="p-rateing  d-flex">
                                                        ...
                                                        <p class="mb-0 ml-3"><span class="t-gray">
                                                                {{ $product_rating->created_at->diffForHumans() }} </span>
                                                        </p>
                                                    </div>
                            ...
                            <div class="card card-product">
                                ...
                                <div class="card-body pt-0">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        @else
                                            <a href="{{ route('store.product.product_view', [$store->slug, $product->id]) }}"
                                                class="action-item pcart-icon bg-primary" data-id="{{ $product->id }}">
                                                {{ __('Add To Cart') }}
                                                <i class="ml-2 fas fa-shopping-basket"></i>
                                            </a>
                                        @endif
